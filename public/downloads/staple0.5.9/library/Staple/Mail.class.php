...
class Staple_Mail
{
	...
	public function __construct($to = NULL, $from = NULL, array $cc = array(), array $bcc = array())
	{
		...
		if(file_exists(CONFIG_ROOT.'application.ini'))
		{
			...
			if($this->checkConfig($curConfig['email']))
			{
				$settings = $curConfig['email'];
				if($settings['from'] != '')
				{
		...
		//@todo Add functionality to load default values from the application.ini
	}
	...
	public function Email($to = NULL, $subject = NULL, $body = NULL, $from = NULL, array $cc = array(), array $bcc = array())
	{
		...
		if($this->checkMailRequiredFields())
		{
			...
			return mail($toList,$this->getSubject(),$this->getBody(),$headers);
		}
		return false;
	}
	...
	public function Text($to = NULL, $subject = NULL, $body = NULL, $from = NULL)
	{
		...
		if($this->checkMailRequiredFields())
		{
			...
			$sent = mail($toList,$subject,$body,$headers);
			return true;
		}
		return false;
	}
