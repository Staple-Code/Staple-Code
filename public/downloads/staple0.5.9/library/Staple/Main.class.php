...
class Staple_Main
{
	...
	{
		//Changes to true when a valid page can be found.
		$dispatch = false; 
		...
		if(file_exists($scriptRoute))
		{
			...
			if(ctype_alnum(str_replace('/','',self::$route)))
			{
				...
				if(Staple_Config::getValue('auth', 'enabled') != 0)
				{
					...
					if(in_array(self::$route, Staple_Config::getValue('auth', 'allowedRoute')) === true)
					{
	 ...
	 * @param array $params
	 */
	protected function dispatchController($controller,$action, array $params)
	{
